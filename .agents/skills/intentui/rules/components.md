...
Always use Intent UI components from `web/components/ui/` instead of raw HTML elements. Import from `@/components/ui/<component-name>`.
...
| `<a>` (standalone, non-Inertia) | `<Link>` | `@/components/ui/link` |
...
For internal SPA navigation and backend-connected actions in this repository, use Inertia primitives with Wayfinder helpers instead of replacing them with generic UI links.

```tsx
import { Link } from "@inertiajs/react"
import { show } from "@/actions/App/Http/Controllers/UserController"

<Link href={show(user.id)}>View user</Link>
```

Use Intent UI link components for styled content links or non-Inertia links when that is the established pattern.
...
- In this repository, these imports resolve into the `web/` frontend tree
