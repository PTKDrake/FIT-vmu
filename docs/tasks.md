...
| 0.4 | Setup | GitHub Actions test workflow | done | Codex | `chore/github-actions-tests` |  | `.github/workflows/tests.yml`, `docs/tasks.md` | `python3 -c "import pathlib, yaml; yaml.safe_load(pathlib.Path('.github/workflows/tests.yml').read_text())"`, `./phpw artisan test --compact`, `pnpm check` | Tạo CI workflow cho PHP 8.5, SQLite, Pint, Larastan, Pest, và frontend checks ở root |
...
- [ ] Không trùng owner với task đang sửa cùng file nhạy cảm.
- [ ] Có branch name.
...
- Khi bắt đầu task: chuyển `todo` sang `doing`, điền `Owner` và `Branch`.
...
- Mỗi task chỉ có một owner chính.
