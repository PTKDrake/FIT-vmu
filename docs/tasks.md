...
| 3.1 | Backend | Foundation data objects | done | `feature/backend-data-objects` |  | `app/Data/*`, `tests/Unit/Data/FoundationDataObjectsTest.php`, `docs/tasks.md` | `vendor/bin/pint --dirty --format agent`, `./phpw artisan test --compact tests/Unit/Data/FoundationDataObjectsTest.php` | Đã thêm 6 Spatie Data object nền tảng với `fromModel()` để map từ Eloquent model sang DTO camelCase và có test đơn vị xác nhận cast/nullable |
| 3.2 | Backend | Posts policy and form requests | done | `feature/posts-policy-validation` |  | `app/Policies/PostPolicy.php`, `app/Http/Requests/Posts/*`, `tests/Feature/Posts/*`, `docs/tasks.md` | `vendor/bin/pint --dirty --format agent`, `./phpw vendor/bin/pest tests/Feature/Posts/PostPolicyTest.php tests/Feature/Posts/PostRequestTest.php` | Đã thêm `PostPolicy` theo permission matrix, 3 form request cho store/update/publish với `toDto()` trả `PostData`, và test bao phủ authorization + validation |
| 3.3 | Backend | Documents policy and form requests | done | `feature/documents-policy-validation` |  | `app/Policies/DocumentPolicy.php`, `app/Http/Requests/Documents/*`, `tests/Feature/Documents/*`, `docs/tasks.md` | `vendor/bin/pint --dirty --format agent`, `./phpw vendor/bin/pest tests/Feature/Documents/DocumentPolicyTest.php tests/Feature/Documents/DocumentRequestTest.php` | Đã thêm policy cho quản lý/view/download document theo visibility và permission, cùng request classes store/update/publish trả `DocumentData` |
| 3.4 | Backend | Query builder for CMS lists | done | `feature/cms-query-builders` |  | `app/QueryBuilders/*`, `tests/Feature/QueryBuilders/CmsListQueryTest.php`, `docs/tasks.md` | `vendor/bin/pint --dirty --format agent`, `./phpw vendor/bin/pest tests/Feature/QueryBuilders/CmsListQueryTest.php` | Đã thêm query objects cho list CMS của posts/documents với whitelist filter/sort/include rõ ràng và test case cho allowed/disallowed query params |
