...
Repo hiện đã có nền tảng khá đầy đủ cho CMS, authorization và content domain. Public-facing content đã bắt đầu dùng dữ liệu thật cho page theo slug, nhưng các module public khác vẫn còn sớm.
...
- domain cho navigation, users/roles nhưng một số màn vẫn đang ở mức placeholder
...
- users index đã có filter, sort, paginate và lấy role thật từ DB
- roles/permissions index đã đọc trực tiếp role + permission thật từ DB
- phần tạo/cập nhật/xóa user, role và permission vẫn đang tiếp tục hoàn thiện theo từng flow
...
- đơn vị public
- tài liệu public
- navigation public lấy từ DB theo layout toàn site
...
- `navigation`, `users`, `roles-permissions` mới hoàn thiện một phần
