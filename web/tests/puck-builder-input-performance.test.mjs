...
test("page builder keeps typing changes local to Puck until save", () => {
    ...
    assert.match(
        pageBuilderSource,
        /if \(onChange === undefined\) \{\s+return;\s+\}/,
    );
    assert.doesNotMatch(pageBuilderSource, /onPublish=\{handleSave\}/);
    assert.match(
        pageBuilderPageSource,
        /const \[isDirty, setIsDirty\] = useState\(false\);/,
    );
    assert.match(pageBuilderPageSource, /onDirtyChange=\{setIsDirty\}/);
    assert.doesNotMatch(
        pageBuilderPageSource,
        /draftJson|savedJson|setDraftJson|setSavedJson/,
    );
...
test("layout builder no longer splits and stores slot JSON on every input change", () => {
    ...
    assert.match(
        layoutBuilderSource,
        /if \(callback === undefined\) \{\s+return;\s+\}/,
    );
    assert.doesNotMatch(
        layoutBuilderSource,
